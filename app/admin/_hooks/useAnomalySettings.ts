...
export function useAnomalySettings(ulp: string) {
  ...
  const fetch = useCallback(async () => {
    ...
    const { data, error } = await supabaseBrowser
      ...
      .select("max_beban_trafo_pct, max_arus_jurusan_a, max_unbalance_pct, max_suhu_trafo_c, min_arus_nominal_pct, max_arus_nominal_pct, min_kva_trafo, max_kva_trafo")
      .eq("ulp", ulpKey)
      .maybeSingle();
    ...
    if (!error && data) {
      setSettings({
        ...
        min_arus_nominal_pct: data.min_arus_nominal_pct ?? null,
        max_arus_nominal_pct: data.max_arus_nominal_pct ?? null,
