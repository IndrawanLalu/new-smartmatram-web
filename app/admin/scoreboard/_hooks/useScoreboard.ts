...
export function useScoreboard(bulan: number, tahun: number, ulp: string) {
  ...
  const addItem = async (
    ...
    const { error: err } = await supabaseBrowser.from("lead_measure_items").insert({
      lead_measure_id, nama_item, satuan,
  ...
  return { data, loading, error, refresh, addLM, deleteLM, addItem, deleteItem, updateRealisasi, updateKomitmen, updateLM, updateItemMeta, updateTarget, duplicateToMonth };
}
