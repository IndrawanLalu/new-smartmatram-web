...
export default function InspeksiPanel({ items, loading }: Props) {
  ...
  return (
    <div className={`flex flex-col ${CARD}`}>
      {/* Header */}
      <div className="bg-navy-700 px-3 py-2.5 shrink-0 flex items-center gap-2">
      ...
      {/* KPI chips */}
      <div className="flex gap-2 px-3 py-2 border-b border-line shrink-0">
        ...
        {loading && items.length === 0 ? (
          <div className="p-3 space-y-2">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="h-8 bg-line animate-skeleton rounded-lg" />
            ))}
          </div>
        ) : items.length === 0 ? (
          <div className="flex items-center justify-center h-12 text-xs text-ink-soft">Tidak ada data</div>
        ) : (
          <ul className="divide-y divide-line">
            {items.map((item) => (
              <li key={item.id} className="px-3 py-2 hover:bg-surface transition-colors">
                <div className="flex items-start gap-2">
                  <span className={`mt-1 w-2 h-2 rounded-full shrink-0 ${STATUS_DOT[item.status] ?? "bg-ink-muted"}`} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <span className="text-xs font-medium text-ink truncate max-w-[130px]">
                        {item.lokasi ?? item.penyulang ?? "—"}
                      </span>
                      <span className={`text-[10px] px-1.5 py-0.5 rounded-md font-medium shrink-0 ${STATUS_BADGE[item.status] ?? "bg-line text-ink-soft"}`}>
                        {item.status}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 mt-0.5">
                      {item.ulp && <span className="text-[10px] text-ink-soft">{item.ulp}</span>}
                      <span className="text-[10px] text-ink-soft font-mono ml-auto">
