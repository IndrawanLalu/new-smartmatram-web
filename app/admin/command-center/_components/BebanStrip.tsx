  ...
  return (
    <div className={`${CARD} shrink-0`}>
      ...
      <div className="bg-navy-700 px-3 py-2 flex items-center gap-2">
        <BarChart2 size={13} className="text-white/70" />
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          <span className="text-white/70 text-xs font-mono">{latestData.length} gardu</span>
        </div>
      </div>
      ...
      <div className="px-3 py-2.5 grid grid-cols-1 gap-1.5">
        {top8.length === 0 ? (
          <div className="text-xs text-ink-soft text-center py-3">Belum ada data pengukuran</div>
        ) : (
          top8.map((g) => (
            <div key={g.id} className="flex items-center gap-2">
              <span className="text-[10px] font-mono text-ink-soft w-20 shrink-0 truncate">{g.no_gardu}</span>
              ...
              <span className="text-[10px] text-ink-soft font-mono w-14 text-right shrink-0 hidden lg:block">
